...
Work Completed
    ...
    The deck API was divided into two response types. The list endpoint now returns deck summaries for selection, while the single-deck endpoint returns full deck details including associated cards.
...
The current study workflow supports:
    ...
    Test coverage was also expanded during this sprint. Controller tests were updated to reflect the summary/detail API split, and service-layer unit tests were added for DTO mapping and missing-deck behavior.
...
Challenges
    ...
    Another challenge was shaping the API to support both deck selection and card study cleanly. This led to separating summary responses from full deck detail responses.
...
Decisions Made
    ...
    Deck responses were split into summary and detail DTOs so the deck selection screen would not depend on loading full card data for every deck.
